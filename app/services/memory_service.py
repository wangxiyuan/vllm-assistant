...
class MemoryService:
    ...
    def sync_code_knowledge(self, changed_files: List[Dict[str, Any]]) -> Dict[str, int]:
        """代码同步后增量更新知识库。

        Args:
            changed_files: 变更的文件列表，每项含 {
                "file_path": str,
                "repo": str,
                "content": str,
                "checksum": str,
        ...
        for f in changed_files:
            ...
            if action == "removed":
                ...
                existing = self.find_by_source_ref(f["file_path"])
                if existing:
                    self.forget(existing.id, hard_delete=False)
                    stats["removed"] += 1
                continue
            ...
            if f["file_path"].endswith((".md", ".rst", ".txt")):
                knowledge = self._extract_doc_structure(f["file_path"], content)
                source_type = "docs"
                tags = ["docs", f.get("repo", "unknown")]
            else:
                knowledge = self._extract_code_structure(f["file_path"], content)
                source_type = "code_structure"
                tags = ["code", f.get("repo", "unknown"), self._file_module(f["file_path"])]

            if not knowledge:
                continue

            if existing:
                self.update(existing.id, content=knowledge, checksum=checksum)
                stats["updated"] += 1
            else:
                self.remember(
                    content=knowledge,
    ...
    def cleanup_stale(self, days_threshold: int = 90) -> int:
        """标记超过 N 天未访问的知识为 stale（保留但降权）"""
        ...
        try:
            count = (
                db.query(AIMemory)
                .filter(
                    AIMemory.last_accessed_at < cutoff,
                    AIMemory.is_stale == False,
                )
                .update(
                    {"is_stale": True, "updated_at": datetime.now(timezone.utc).replace(tzinfo=None)},
            ...
            db.commit()
            if count:
                logger.info(f"Marked {count} knowledge entries as stale")
            return count
        except Exception:
            logger.exception("Failed to cleanup stale knowledge")
            db.rollback()
            return 0
        finally:
            db.close()
