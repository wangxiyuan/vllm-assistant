    ...
    <div class="community-list">
      <div v-for="item in displayedItems" :key="item.type + '-' + (item.number || item.pr_number)"
        ...
        <div class="item-actions">
          <button class="btn btn-sm" :class="(item.type === 'pr' ? watchlistStore.findWatchlistItem(item.number || item.pr_number, 'pr') : watchlistStore.findWatchlistItem(item.number, 'issue')) ? 'btn-starred' : 'btn-ghost'"
            ...
            <svg width="12" height="12" viewBox="0 0 24 24" :fill="(item.type === 'pr' ? watchlistStore.findWatchlistItem(item.number || item.pr_number, 'pr') : watchlistStore.findWatchlistItem(item.number, 'issue')) ? 'currentColor' : 'none'" stroke="currentColor" stroke-width="2">
