...
const navGroups = ref<NavGroup[]>([
  {
    ...
    expanded: true,
    items: [
      { name: 'overview', label: '总览', kbd: '1', icon: 'M3 3h7v9H3zM14 3h7v5h-7zM14 12h7v9h-7zM3 16h7v5H3z' },
    ],
  },
  {
    label: '任务',
    ...
    <div v-if="appStore.mobileMenuOpen" class="mobile-overlay" @click="appStore.mobileMenuOpen = false">
      <div class="mobile-sidebar" @click.stop>
        ...
        <nav class="nav-section">
          ...
          <button class="nav-item" @click="slackStore.openManager(); appStore.mobileMenuOpen = false">
            <svg class="nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
